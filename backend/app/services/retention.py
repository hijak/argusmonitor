    ...
    for policy in policies:
        ...
        log_result = await db.execute(
            delete(LogEntry).where(
                LogEntry.workspace_id == policy.workspace_id,
                LogEntry.timestamp < thresholds["logs"],
            )
        )
        ...
        service_metric_result = await db.execute(
            delete(ServiceMetric).where(
                ServiceMetric.workspace_id == policy.workspace_id,
                ServiceMetric.recorded_at < thresholds["metrics"],
            )
        )
        alert_result = await db.execute(
            delete(AlertInstance).where(
                AlertInstance.workspace_id == policy.workspace_id,
                AlertInstance.created_at < thresholds["alerts"],
            )
        )
        incident_result = await db.execute(
            delete(Incident).where(
                Incident.workspace_id == policy.workspace_id,
                Incident.started_at < thresholds["incidents"],
            )
        )
        run_result = await db.execute(
            delete(TransactionRun).where(
                TransactionRun.workspace_id == policy.workspace_id,
                TransactionRun.started_at < thresholds["runs"],
            )
        )

        results["logs"] += log_result.rowcount or 0
        results["metrics"] += (metric_result.rowcount or 0) + (service_metric_result.rowcount or 0)
