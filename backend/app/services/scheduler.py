...
def start_scheduler() -> BackgroundScheduler:
    ...
    scheduler.add_job(enqueue_recurring_jobs, "interval", seconds=60, id="enqueue_jobs", max_instances=1)
    scheduler.add_job(run_worker_tick, "interval", seconds=15, id="worker_tick", max_instances=1)
    scheduler.add_job(run_retention_tick, "interval", hours=6, id="retention_tick", max_instances=1)
    scheduler.start()
    return scheduler
