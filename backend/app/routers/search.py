...
from app.models import Host, Service, Dashboard, Transaction, AlertRule, User
...
async def global_search(
    ...
    hosts = (await db.execute(
        ...
        .where(or_(Host.name.ilike(term), Host.ip_address.ilike(term)))
        .limit(8)
    )).all()
    for h in hosts:
        results.append({
    ...
    services = (await db.execute(
        ...
        .where(or_(Service.name.ilike(term), Service.url.ilike(term)))
        .limit(5)
    )).all()
    for s in services:
        results.append({
    ...
    dashboards = (await db.execute(
        ...
        .where(Dashboard.name.ilike(term))
        .limit(5)
    )).all()
    for d in dashboards:
        results.append({
    ...
    transactions = (await db.execute(
        ...
        .where(Transaction.name.ilike(term))
        .limit(5)
    )).all()
